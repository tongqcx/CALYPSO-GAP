...
SUBROUTINE FCAR2WACSF(elements, lat, pos)
character(2), intent(in)                :: elements
REAL(DP),intent(in),dimension(3,3)      :: lat
REAL(DP),intent(in),dimension(:,:)      :: pos
!local
at%natoms = size(pos,1)
call ini_structure(at, data_c)
do i = 1,3
    at%lat(i,:) = lat(i,:)
enddo
do i = 1, natoms
    at%symbols(i) = elements(i)
    at%pos(i,:) = pos(i,:)
enddo
call build_neighbor(at, data_c)
call car2acsf(at, GAP, ACSF, DATA_C)
