  subroutine olinmin(xparam,step,pvect,nvar,nmin,funct1,okf,grad,imode, na, species, extstress)
!*********************************************************************
!
...
!  print = .TRUE.
  ...
  eps = 0.00001_dp
  !print*, '******************'
  !print*, 'vprec', index(keyword,'vprec')
  !print*, 'lreaxFF', lreaxFF
  !print*, 'prec', index(keyword,'prec')
  !print*, index(keyword,'vprec').ne.0.or.lreaxFF
  !print*, index(keyword,'prec').ne.0
  !print*, '******************'
...
!
  ...
  fmin = funct1
  step = vt(2)
  !xparam(1:nvar) = xparam(1:nvar) + step*pvect(1:nvar)
  call daxpy(nlvar,step,pvect(nmin),1_i4,xparam(nmin),1_i4)
  !print*, step
  !stop
  iflag = 0
  if (imode.eq.1) then
    call funct(iflag,nvar,xparam,phi(2),grad, na, species, extstress)
  !  print*, 'XXX',phi(2),xparam(1:3)
  else
...
50 stlast = vt(3)
  ...
  if (imode.eq.1) then
    ...
    call funct(iflag,nvar,xparam,funct1,grad, na, species, extstress)
  !  print*, 'XXC',funct1,xparam(1:3)
  else
