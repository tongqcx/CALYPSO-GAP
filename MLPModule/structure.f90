...
use math
implicit none
integer,parameter                      :: max_neighbor = 11000
type Atoms
    character(2)                       :: name
...
enddo
write(181, *) '********************************************'
write(181, '(A35, F10.5)') 'RMSE ENERGY (Units eV/atom):', sqrt(rmse_energy/n_config)
write(181, '(A35, F10.5)') 'RMSE FORCE (Units eV/angstrom):', sqrt(rmse_force/nforce)
write(181, '(A35, F10.3)') 'RMSE STRESS (Units GPa):', sqrt(rmse_stress/n_config/6.d0)
write(181, *) '********************************************'
close(181)
END SUBROUTINE
