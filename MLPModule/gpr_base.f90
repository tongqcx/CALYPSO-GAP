...
SUBROUTINE GPR2(cmm, cmo, lamdaobe, alpha)
implicit none
real(dp),intent(in),dimension(:,:)      :: cmm
real(dp),intent(in),dimension(:,:)      :: cmo
real(dp),intent(in),dimension(:)        :: lamdaobe
...
integer                                 :: i,j
real(dp)                                :: max_qmm
real(dp),allocatable,dimension(:,:)     :: tmm
real(dp),allocatable,dimension(:,:)     :: qmm, invqmm
real(dp),allocatable,dimension(:,:)     :: tmo, tobe, talpha

n_globalSparseX = size(cmo, 1)
n_globalY = size(cmo, 2)

if (.not.allocated(tmm))    allocate(tmm(n_globalSparseX, n_globalSparseX))
if (.not.allocated(qmm))    allocate(qmm(n_globalSparseX, n_globalSparseX))
if (.not.allocated(invqmm)) allocate(invqmm(n_globalSparseX, n_globalSparseX))
if (.not.allocated(tobe))   allocate(tobe(n_globalY,1))
if (.not.allocated(talpha)) allocate(talpha(n_globalSparseX,1))
if (.not.allocated(tmo))    allocate(tmo(n_globalSparseX, 1))

call matmul_real(cmo, transpose(cmo),'N', 'N', tmm)
qmm = cmm + tmm
deallocate(tmm)
max_qmm = maxval(qmm)
qmm = qmm/max_qmm
call my_inverse(n_globalSparseX, qmm, invqmm)
deallocate(qmm)
invqmm = (1.d0/max_qmm) * invqmm
do i = 1, n_globalY
    tobe(i,1) = lamdaobe(i)
enddo
call matmul_real(cmo, tobe, 'N','N',tmo)
call matmul_real(invqmm, tmo, 'N', 'N', talpha)
deallocate(tmo)

do i = 1, n_globalSparseX
    alpha(i) = talpha(i,1)
enddo
deallocate(invqmm, tobe, talpha)
END SUBROUTINE
