...
write(*,'(A30X2I10)'), 'The size of globalY:', GAP%nglobalY, DATA_C%nob
write(*,'(A30XF10.5)'), 'SparseCut:', GAP%sparse_dis_len
write(*,'(A30XI5)'), 'Isparse:',GAP%sparse_method
write(*,'(A30XI7)'), 'The size of sparse set:', GAP%nsparse
write(*,'(A30XF10.5)'), 'sigma_atom:',GAP%sigma_atom
...
!    call write_array(GAP%cmm, 'cmm.datx')
    ...
    if (GAP%sparse_method == 2) then
        ...
        if (GAP%nsparse > 120 .and. GAP%nsparse < 200) then
            ...
            print*, 'SparseCut:', GAP%sparse_dis_len
        ...
        elseif (GAP%nsparse >= 200) then
            ...
            print *,'Increasing SparseCut to:',GAP%sparse_dis_len
        ...
        elseif (GAP%nsparse <= 120 )then
            ...
            print *,'*Decreasing SparseCut to:',GAP%sparse_dis_len
        ...
        if (det_cmm > Inverse_error_min) then
            ...
            print *,'Increasing SparseCut',GAP%sparse_dis_len, GAP%nsparse
            if (allocated(GAP%cmm))  deallocate(GAP%cmm)
            if (allocated(calc_det))  deallocate(calc_det)
        endif
    endif
...
SUBROUTINE GAP_WRITE_PARAS_MB(GAP, ACSF)
type(GAP_type),intent(in)             :: GAP
type(ACSF_type),intent(in)            :: ACSF

open(2234,file='gap_parameters')
!
! write species information
!
write(2234,'(I5)') DATA_C%nspecies
do i = 1, DATA_C%nspecies
    write(2234,'(I5, F15.5)'), get_atom_number(DATA_C%elements(i)), DATA_C%elements_weight(i)
enddo
!
! write wacsf information
!
write(2234,'(I5)')  acsf%nsf
do i = 1, acsf%nsf
    write(2234,'(I3, F10.5, F10.5)') acsf%sf(i)%ntype, acsf%sf(i)%alpha, acsf%sf(i)%cutoff
enddo
!
! write gpr information
!
...
if(.not.alive) then
    print*, "FILES-> gap_parameters does not exist!"
    stop
endif
open(2234,file='gap_parameters')

read(2234,*) DATA_C%nspecies
do i = 1, DATA_C%nspecies
    read(2234,*)  atom_number , weight
    DATA_C%elements_weight(i) = weight
enddo
read(2234,*) acsf%nsf
allocate(acsf%sf(acsf%nsf))
do i = 1, acsf%nsf
    read(2234,*) acsf%sf(i)%ntype, acsf%sf(i)%alpha, acsf%sf(i)%cutoff
enddo
